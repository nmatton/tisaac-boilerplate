...
var EasyPZLoader = /** @class */ (function () {
  ...
  EasyPZLoader.getSettingsFromString = function (settingsString) {
    ...
    if (!!window['onPanned']) settings.onPanned = window['onPanned'];
    if (!!window['onZoomed']) settings.onZoomed = window['onZoomed'];
    if (!!window['onTransformed']) settings.onTransformed = window['onTransformed'];
    if (!!window['onResetAbsoluteScale']) settings.onResetAbsoluteScale = window['onResetAbsoluteScale'];
    var settingsObj = {};
    try {
    ...
    if (settingsObj['replaceVariables']) settings['replaceVariables'] = !!settingsObj['replaceVariables'];
    return settings;
  };
  EasyPZLoader.prototype.checkElements = function () {
    ...
    var prevEls = this.easyPzElements.map(function (obj) {
      return obj.element;
    });
...
var EasyPZ = /** @class */ (function () {
  function EasyPZ(
    ...
    if (onTransform === void 0) {
      onTransform = function () {};
    }
    if (modeSettings === void 0) {
      modeSettings = {};
    }
    if (onPanned === void 0) {
      onPanned = function () {};
    ...
    if (onResetAbsoluteScale === void 0) {
      onResetAbsoluteScale = function () {};
    }
    if (applyTransformTo === void 0) {
      applyTransformTo = '';
    }
  ...
  EasyPZ.prototype.setSettings = function (
    onTransform,
    options,
    enabledModes,
    modeSettings,
    ...
    if (options) {
      ...
      if (typeof options.elt !== 'undefined') {
        this.options.elt = options.elt;
        this.eltWidth = options.elt.offsetWidth;
        this.eltHeight = options.elt.offsetHeight;
      }
    ...
    this.resetAbsoluteScale.subscribe(function () {
      return _this.saveCurrentTransformation(onResetAbsoluteScale);
    });
    this.onPanned.subscribe(function () {
      return _this.applyTransformation();
    });
    this.onZoomed.subscribe(function () {
      return _this.applyTransformation();
    });
  };
  ...
  EasyPZ.prototype.trackTotalTransformation = function (onTransform, onPanned, onZoomed, transformBeforeScale) {
    ...
    this.onZoomed.subscribe(function (zoomData) {
      ...
      if (transformBeforeScale) {
        _this.totalTransform.translateX =
          ((relativeTransform.translateX - zoomData.x) / scalePrev) * _this.totalTransform.scale + zoomData.x;
        _this.totalTransform.translateY =
          ((relativeTransform.translateY - zoomData.y) / scalePrev) * _this.totalTransform.scale + zoomData.y;
    ...
    this.fitInAndCenter = function () {
      let width = _this.eltWidth;
      let widthScale = (_this.width - this.options.padding.left + this.options.padding.right) / width;
      let height = _this.eltHeight;
      let heightScale = (_this.height - this.options.padding.top - this.options.padding.bottom) / height;
      let scale = Math.min(widthScale, heightScale);
      _this.totalTransform.scale = scale;
      _this.totalTransform.translateX = (this.width - this.eltWidth * scale) / 2 + this.options.padding.left;
      _this.totalTransform.translateY = (this.height - this.eltHeight * scale) / 2 + this.options.padding.top;
      onTransform(_this.totalTransform);
    };
  };
  EasyPZ.prototype.getScaleWithinLimits = function (scale) {
    ...
    if (this.options.bounds && this.options.elt) {
      let bounds = this.options.bounds;
      let width = this.eltWidth; // + bounds.maxLeft + bounds.maxRight;
      let widthScale = (this.width - this.options.padding.left + this.options.padding.right) / width;
      let height = this.eltHeight; // + bounds.maxTop + bounds.maxBottom;
      let heightScale = (this.height - this.options.padding.top - this.options.padding.bottom) / height;
  ...
  EasyPZ.prototype.ensureTransformWithinBounds = function (transformBeforeScale) {
    ...
    const boundsLeft = [bounds.minLeft * scale, bounds.maxLeft * scale];
    if (x < boundsLeft[0]) x = boundsLeft[0];
    else if (x > boundsLeft[1]) x = boundsLeft[1];

    const boundsRight = [
      this.width - (this.eltWidth + bounds.minRight) * scale,
      this.width - (this.eltWidth + bounds.maxRight) * scale,
    ];
    if (x < boundsRight[1]) x = boundsRight[1];
    else if (x > boundsRight[0]) x = boundsRight[0];

    const boundsTop = [bounds.minTop * scale, bounds.maxTop * scale];
    if (y < boundsTop[0]) y = boundsTop[0];
    else if (y > boundsTop[1]) y = boundsTop[1];

    const boundsBottom = [
      this.height - (this.eltHeight + bounds.minBottom) * scale,
      this.height - (this.eltHeight + bounds.maxBottom) * scale,
    ];
    if (y < boundsBottom[1]) y = boundsBottom[1];
    else if (y > boundsBottom[0]) y = boundsBottom[0];
    ...
    if (this.applyTransformTo) {
      ...
      for (var i = 0; i < els.length; i++) {
        ...
        if (transformData) {
          ...
          originalTranslate.x =
            (transformData.translateX - this.lastAppliedTransform.translateX / originalScaleX) * translateBeforeScaleFactorX;
          originalTranslate.y =
            (transformData.translateY - this.lastAppliedTransform.translateY / originalScaleY) * translateBeforeScaleFactorY;
  ...
  EasyPZ.prototype.applyModeSettings = function (modeSettings) {
    ...
    if (modeNames && modeNames.length) {
      var _loop_1 = function (modeName) {
        var modes = this_1.modes.filter(function (m) {
          return m.ids.indexOf(modeName) !== -1;
        });
        if (modes.length !== 1) {
          ...
          for (var settingName in newSettings) {
            if (!mode.settings[settingName]) {
              console.error(
                'Trying to set a setting for the easypz mode ',
                modeName,
                ', but this setting does not exist: ',
                settingName
              );
            } else {
  ...
  EasyPZ.easeInteraction = function (maxSpeed, duration, onStep) {
    return EasyPZ.momentumInteraction(
      onStep,
      function (timePassed) {
  ...
  EasyPZ.isRightClick = function (event) {
    return (
      ...
      !!event.clientX &&
      (('which' in event && event.which === 3) || ('button' in event && event.button === 2))
    );
  };
...
EasyPZ.addMode(function (easypz) {
  var mode = {
    ...
    settings: {
      maxDistance: 3,
      delay: 350,
      zoomInScaleChangePerMs: -0.0015,
      zoomOutScaleChangePerMs: 0.003,
    ...
    onClickTouch: function (eventData) {
      ...
      var recursiveZoom = function () {
        if (mode.active) {
          ...
          var scaleChangePerMs = mode.data.zoomingOut
            ? mode.settings.zoomOutScaleChangePerMs
            : mode.settings.zoomInScaleChangePerMs;
      ...
      easypz.callbackAfterTimeoutOrMovement(mode.settings.delay, mode.settings.maxDistance).then(function (dist) {
        ...
        if (mode.active) {
          ...
          // start zooming
          var clickFirst = eventData.modeName === 'CLICK_HOLD_ZOOM_IN' || eventData.modeName === 'CLICK_HOLD_ZOOM_OUT';
          if (hasClickedFirst == clickFirst) {
            mode.data.zoomingOut = eventData.modeName === 'HOLD_ZOOM_OUT' || eventData.modeName === 'CLICK_HOLD_ZOOM_OUT';
            recursiveZoom();
          }
        }
      });
...
EasyPZ.addMode(function (easypz) {
  var mode = {
    ...
    settings: { dblClickTime: 300, zoomInScaleChange: 0.33, zoomOutScaleChange: 3, maxHoldTime: 200 },
  ...
  var mode = {
    ...
    onMove: function (eventData) {
      if (mode.data.posStart1 && mode.data.posStart2) {
        ...
        mode.data.zoomPos = {
          x: (mode.data.posStart1.x + mode.data.posStart2.x) / 2,
          y: (mode.data.posStart1.y + mode.data.posStart2.y) / 2,
        };
        ...
        easypz.onZoomed.emit({
          ...
          absoluteScaleChange: scale,
          targetX: mode.data.zoomCenterPos.x,
          targetY: mode.data.zoomCenterPos.y,
        });
      }
    },
    onClickTouchEnd: function (eventData) {
      ...
      if (eventData.modeName === 'PINCH_ZOOM_MOMENTUM' && mode.data.references.length > 5) {
        ...
        mode.data.momentum = EasyPZ.frictionInteraction(absScaleChangeSpeed, mode.settings.friction, function (_a) {
          ...
          easypz.onZoomed.emit({
            x: _this.pinchZoomPos.x,
            y: _this.pinchZoomPos.y,
            absoluteScaleChange: newScale,
            targetX: _this.pinchZoomCenterPos.x,
            targetY: _this.pinchZoomCenterPos.y,
          });
    ...
    onClickTouch: function () {
      if (easypz.numberOfPointers !== 1) {
        ...
        easypz.callbackAfterTimeoutOrMovement(mode.settings.delay, mode.settings.minDistance).then(function (dist) {
          ...
          if (!mode.active && mode.data.el) mode.data.el.style.display = 'none';
        });
        setTimeout(function () {
          if (easypz.numberOfPointers !== 1) {
            mode.active = false;
            if (mode.data.el) mode.data.el.style.display = 'none';
          }
        }, mode.settings.delay);
      }
    },
    ...
    onClickTouchEnd: function (eventData) {
      if (mode.active) {
        ...
        if (Date.now() - easypz.mouseDownTime > mode.settings.minTime) {
          var middle = {
            x: mode.data.startPos.x + (easypz.mousePos.x - mode.data.startPos.x) / 2,
            y: mode.data.startPos.y + (easypz.mousePos.y - mode.data.startPos.y) / 2,
          };
          var dist = {
          ...
          easypz.onZoomed.emit({
            ...
            targetX: _this.width / 2,
            targetY: _this.height / 2,
          });
        }
      }
    },
  ...
  var mode = {
    ...
    onClickTouch: function (eventData) {
      ...
      easypz.callbackAfterTimeoutOrMovement(mode.settings.delay, mode.settings.minDistance).then(function (dist) {
        ...
        if (
          easypz.numberOfPointers > 1 ||
          dist < mode.settings.minDistance ||
          distInDirection / dist < mode.settings.minDirectionPercentage
        ) {
          mode.active = false;
    ...
    onMove: function (eventData) {
      if (mode.active) {
        ...
        if (eventData.modeName === 'DYNAMIC_ZOOM_X_ADJUSTABLE' || eventData.modeName === 'DYNAMIC_ZOOM_Y_ADJUSTABLE') {
          actualZoomPosition = {
            x: mode.data.startPos.x + mode.data.relativePos.x,
            y: mode.data.startPos.y + mode.data.relativePos.y,
          };
          targetX = easypz.mousePos.x;
          targetY = easypz.mousePos.y;
          dynamicZoomPosition = { x: easypz.mousePos.x, y: easypz.mousePos.y };
        } else if (eventData.modeName === 'DYNAMIC_ZOOM_X_STATIC' || eventData.modeName === 'DYNAMIC_ZOOM_Y_STATIC') {
          dynamicZoomPosition = { x: mode.data.startPos.x, y: mode.data.startPos.y };
        } else if (eventData.modeName === 'DYNAMIC_ZOOM_X_NORMAL_PAN' || eventData.modeName === 'DYNAMIC_ZOOM_Y_NORMAL_PAN') {
          dynamicZoomPosition = {
            x: mode.data.startPos.x - mode.data.relativePos.x,
            y: mode.data.startPos.y - mode.data.relativePos.y,
          };
        } else if (eventData.modeName === 'DYNAMIC_ZOOM_X_ORIGINAL_PAN' || eventData.modeName === 'DYNAMIC_ZOOM_Y_ORIGINAL_PAN') {
          dynamicZoomPosition = { x: easypz.mousePos.x, y: easypz.mousePos.y };
        }
  ...
  var mode = {
    ...
    onMove: function (eventData) {
      if (mode.data.zoomReference) {
        ...
        if (
          !mode.active &&
          mode.data.hasChangedDirection &&
          direction == mode.data.hasChangedDirectionDirection &&
          signNow != mode.data.hasChangedDirectionSign
        ) {
        ...
        if (mode.active && distanceSinceLast) {
          ...
          var zoomPos = {
            x: (easypz.mousePos.x + mode.data.zoomReference.x) / 2,
            y: (easypz.mousePos.y + mode.data.zoomReference.y) / 2,
          };
